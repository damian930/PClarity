...
Data_buffer data_buffer_make(Arena* arena, U64 count)
{
  ...
  buffer.count = count;
  if (count == 0) { buffer.data = 0; } // Just in case
  return buffer;
}
...
// - makers 
// 
Str8 str8_manual_alloc(Arena* arena, U8* str, U64 len)
{
  Str8 result = {};
...
Str8 str8_manual_view(U8* buffer, U64 count)
{
  Str8 str = {};
  str.data = buffer;
  str.count = count;
...
Str8 str8_manual_alloc_nt(Arena* arena, U8* str)
{
  U64 str_len = strlen((char*)str); 
  return str8_manual_alloc(arena, str, str_len);
}

Str8 str8_manual_view_nt(U8* buffer)
{
  U64 str_len = strlen((char*)buffer); 
  return str8_manual_view(buffer, str_len);
}

Str8 str8_from_cstr(Arena* arena, U8* str)
{
  Str8 result = str8_manual_alloc(arena, str, strlen((char*)str));
  return result;
}
...
{
  Str8 str = str8_manual_view(ArenaCurrentAddressP(arena, U8), 0);
  U64 node_index = 0;
...
// - str16 makes 
//
Str16 str16_manual_alloc(Arena* arena, wchar_t* wstr, U64 count)
{
  Str16 str = {};
  str.data = ArenaPushArr(arena, wchar_t, count);
  str.count = count;
  wchar_t* nt = ArenaPush(arena, wchar_t);
  *nt = L'\0';
  return str;
}

Str16 str16_manual_view(wchar_t* wstr, U64 count)
{
  Str16 str = {};
  str.data = wstr;
  str.count = count;
...
Str16 str16_manual_alloc_nt(Arena* arena, wchar_t* wstr)
{
  U64 size = wcslen(wstr);
  return str16_manual_alloc(arena, wstr, size);
}

Str16 str16_manual_view_nt(wchar_t* wstr)
{
  U64 size = wcslen(wstr);
  return str16_manual_view(wstr, size);
}
...
Str8_list str8_list_copy_shallow(Arena* arena, Str8_list other)
{
  ...
  {
    str8_list_append_view(arena, &list, node->str);
  }
  return list;
}
...
void str8_list_append_view(Arena* arena, Str8_list* list, Str8 str) 
{
...
{
  Str8 copy = str8_copy(arena, str);
  str8_list_append_view(arena, list, copy);
}
...
Str8_list str8_split_ex(Arena* arena, Str8 str, Str8 spliter, Str8_match_flags flags, B32 include_empty_once_after_split)
{
  ...
  if (spliter.count <= str.count)
  {
    ...
    if (spliter.count != 0)
    {
      for (U64 i = 0; (i < str.count) && (i + spliter.count <= str.count);)
      {
        ...
        if (str8_match(test_substring, spliter, flags))
        {
          ...
          {
            str8_list_append_view(arena, &result_list, new_substring);
          }
    ...
    str8_list_append_view(arena, &result_list, str);
  }
  return result_list;
}
