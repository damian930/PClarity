...
///////////////////////////////////////////////////////////
// - Bit of helpers here for text encoding
//


Str16 str16_from_str8(Arena *arena, Str8 str8) // todo: Take a look at this, claude made this
{
  int len = MultiByteToWideChar(CP_UTF8, 0, (char*)str8.data, (int)str8.count, Null, 0);
  WCHAR *buf = ArenaPushArr(arena, WCHAR, len + 1);
  MultiByteToWideChar(CP_UTF8, 0, (char*)str8.data, (int)str8.count, buf, len);
  buf[len] = '\0';
  return Str16{ buf, (U64)len };
}

Str8 str8_from_str16(Arena *arena, Str16 str16) // todo: Take a look at this, claude made this
{
  int len = WideCharToMultiByte(CP_UTF8, 0, str16.data, (int)str16.count, Null, 0, Null, Null);
  U8 *buf = ArenaPushArr(arena, U8, len + 1);
  WideCharToMultiByte(CP_UTF8, 0, str16.data, (int)str16.count, (char*)buf, len, Null, Null);
  buf[len] = '\0';
  return str8_manual_view(buf, len);
}
...
B32 os_init()
{
  ...
  { // Making sure that the windows version that this app is going to run on 
    ...
    LONG nt_status = RtlGetVersion(&info);
    Assert(nt_status == 0); // Docs say that this just retrun a const, so just in case i assert

    DWORD major = info.dwMajorVersion;
    DWORD minor = info.dwMinorVersion;
    DWORD build = info.dwBuildNumber;

    // NOTE: This is here for now, but not yet used since i dont know what i need exactly

    if (major < 10) 
    {
      int res = MessageBoxW(
        Null,           
        L"Current windows version is not supported.\nMinimum supported version is Windows 10.", 
        L"Error",       
        MB_OK | MB_ICONERROR 
      );
  ...
  //   Handle(hr == S_OK);
  //   Str8 str = __os_win32_str8_from_utf16_nt(__os_g_state->state_arena, wstr);
  //   __os_g_state->path_to_system_fonts = str;
  // }
 
  __os_g_state->frame_arena = arena_alloc(Kilobytes(64));
...
OS_File os_file_open_ex(Str8 file_name, OS_File_access_flags acess_flags, OS_Error* out_error)
{
  ...
  */
  ...
  // todo: fix the utf8 bug here
  Str16 utf16_nt_file_name = str16_from_str8(scratch.arena, file_name);

  HANDLE file_handle = CreateFileW(
    utf16_nt_file_name.data,
    desired_access,
    share_mode,
    Null,
    creation_dispostion,
  ...
  {
    if (0) {}
    else if (error_code == ERROR_ACCESS_DENIED ) { *out_error = OS_Error__access_denied; } 
    else if (error_code == ERROR_FILE_NOT_FOUND) { *out_error = OS_Error__no_such_path; } 
    else if (error_code == ERROR_ALREADY_EXISTS) { *out_error = OS_Error__already_exists; }
  }

  end_scratch(&scratch);

  OS_File file = os_file_handle_zero();
...
LRESULT win32_proc(
  ...
  switch (message)
  {
    ...
    case WM_CAPTURECHANGED:
    {
      ...
      //       ois now capturing the mouse and we lst the capture.
