...
void ui_init()
{
  ...
  __ui_g_context->style_stacks_arena = arena_alloc(Megabytes(64));
  for EachIndex(i, ArrayCount(__ui_g_context->build_arenas)) { 
    __ui_g_context->build_arenas[i] = arena_alloc_ex(Megabytes(64), true, (i == 0 ? 1 : 44444)); 
  }
...
void ui_begin_build(V2F32 window_dims, V2F32 mouse_pos)
{
  ...
  ctx->interacted_with_box_id = str8_copy(ui_get_build_arena(), ctx->interacted_with_box_id); 
  ctx->active_box_id          = str8_copy(ui_get_build_arena(), ctx->active_box_id); 
  ctx->navigated_box_id       = str8_copy(ui_get_build_arena(), ctx->navigated_box_id); 
...
void ui_draw_box(UI_Box* root, RangeV2F32 parent_scissor_bbox)
{
  ...
  else 
  {
    ...
    if (root->flags & UI_Box_flag__has_text_contents) {
      d_draw_text(root->text, root->font, rect.origin, white_); 
    }
    ...
    if (!range_v2f32_match(new_scissor_bbox, parent_scissor_bbox)) {
      if (rect_from_range_v2f32(new_scissor_bbox).height < 0.0f) { BP; }
